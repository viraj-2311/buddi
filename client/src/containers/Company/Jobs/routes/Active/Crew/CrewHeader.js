...
const CrewHeader = ({
  ...
  const handleTimeChange = (field, time) => {
    ...
    // let dateTime = time.toDate();
    // let timeValue = moment(dateTime).format('HH:mm');
    const payload = {
      [field]: time,
    };
    dispatch(updateJobDetailsRequest(job.id, payload));
    ...
    <CrewHeaderWrapper>
      ...
      <div className='jobContent'>
        <Row gutter={[20, 25]} className='headerTop mb-25'>
          ...
          <Col flex={6}>
            <label className='fieldLabel'>Show Date(s){require &&<span className='helper-text font-star'>*</span>}</label>
            <DayPicker
              className='fieldControl w-180'
              placeholder='Select date'
              month={stringToDate(job.startDate, 'YYYY-MM').toDate()}
              startDate={job?.startDate}
              wrapDate={job?.wrapDate}
              suffixIcon={
                <CalendarIcon width={20} height={18} fill='#bcbccb' />
              }
              ...
              checkIsRequired={(flag) => setRequired(flag)}
            />
            {require && <div className="helper-text lowercase">*Show date(s) required</div>}
          </Col>
          <Col flex={6}>
          ...
          <Col flex={4} className='customFieldControl'>
            ...
            />
            {/* <Input
              className='fieldControl defaultField w-50'
              value={job.soundCheckTime}
              onChange={(time) => handleTimeChange('soundCheckTime', time)}
            /> */}
          </Col>
          <Col flex={5}>
        ...
        <Row gutter={20} className='headerBottom mb-25'>
          ...
          <Col className='rightAction marginBottom'>
            ...
            <Button
              type='default'
              shape='round'
              className='bookCrewBtn'
              onClick={() => {
                if(jobShootDates.length  === 0){
                  setRequired(true)
                } else {
                  onBookCrew();
                }
